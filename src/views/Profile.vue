...
// --- External Profile Logic ---
const externalProfile = ref(null)
const isExternalLoading = ref(false)

const loadExternalProfile = async (email) => {
  // Check if the user is just looking at their own profile via URL
  const myEmail = authState.email || localStorage.getItem('user')
  if (email === myEmail) {
...
const user = computed(() => {
  ...
  return {
    ...
    stats: {
      actions: authState.profile.stats?.actions || 0,
